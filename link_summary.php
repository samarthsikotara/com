...
if(!defined('mnminclude')){header('Location: ../404error.php');die();}
	...
	global $main_smarty, $cached_totals, $new_search, $cached_links, $current_user, $cached_saved_links;
    $link = new Link;
	...
	if ($new_search) {
		...
		if ($links) {
			foreach($links as $link_id) {
				$link->id=$link_id;
				$link->read();
                if($summary_type=="short") $link_summary_output .= $link->print_summary('summary', true, 'link_summary_short.tpl');
                else $link_summary_output .= $link->print_summary('summary', true, 'link_summary.tpl');
