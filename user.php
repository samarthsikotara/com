...
// setup breadcrumbs for the various views
    ...
    if($view=='interest'){
        ...
        foreach($array as $item){
            $details[$i]['id']=$item->interest_id;
            $details[$i]['name']=getInterestDetails($item->interest_id)->interest_name;
            $details[$i]['photo']=my_base_url . my_pligg_base . '/avatars/location_uploaded/'.$item->interest_id.".jpg";
            if(!file_exists($details[$i]['photo'])) $details[$i]['photo']= my_base_url . my_pligg_base . '/avatars/interest_uploaded/default.gif';
            $details[$i]['url']=my_base_url . my_pligg_base . "/interest.php?interestId=".$item->interest_id."&view=info";
            $details[$i]['title']=getInterestDetails($item->interest_id)->interest_short_desc;

            $i++;
        }
        $main_smarty->assign('interest', $details);

    }
   if($view=='location'){
        $array=getLocationList();
        $i=0;
        foreach($array as $item){
            $details[$i]['id']=$item->location_id;
            $details[$i]['name']=getLocationDetails($item->location_id)->location_name;
            $details[$i]['photo']=my_base_url . my_pligg_base . '/avatars/location_uploaded/'.$item->location_id.".jpg";
            if(!file_exists($details[$i]['photo'])) $details[$i]['photo']= my_base_url . my_pligg_base . '/avatars/location_uploaded/default.gif';
            $details[$i]['url']=my_base_url . my_pligg_base . "/location.php?locationId=".$item->location_id."&view=info";
