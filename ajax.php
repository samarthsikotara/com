...
if($work==5){
    ...
    //ToDo: To check if the user can comment on this post.
    ...
    if($cancontinue == true)
	{
        ...
        try{
		    if($comment->store()){
                $return[0]= "success";
                $return[1]= $comment->print_summary("", true);
            }else{
                $return[0]="failed";
            }
        }catch (Exception $e){
...
if($work==11){
    ...
    if(!is_numeric($scribble->groupId)) die('Not a valid community');
...
if($work==12){
    ...
    if($cancontinue == true)
	{
		...
		if($sccomment->store()){
            $return[0]="success";
            $return[1]=$sccomment->print_summary("", true);
        }else{
...
if($work==16){
    ...
    if(isset($_POST) and $_SERVER['REQUEST_METHOD'] == "POST"){
        ...
        if(strlen($name))
        {
            ...
            if($size<(2*1024*1024) && in_array($ext, $valid_formats)) // Image size max 2 MB
            {
            ...
            echo "Only Image file maximum of 2 MB";
            }
        }
        else
...
if($work==54){
    ...
    if(!is_null($result)){
        ...
        foreach($result as $list){
            ...
            $array[$i]['short_desc']="";
            $array[$i]['image']=$base_url.$my_pligg_base."/templates/wistie/images/Maps-32.png";
            $i++;
        }
...
if($work==56){
    ...
    echo $field1=sanitize(strip_tags($_POST['reg_desc1']), 3);
    ...
    if(!is_null($results->user_id)){
       echo  $sql="UPDATE `shbawaras` SET occupation='".$occupation."', workplace='".$place."' ,bawara_field1='".$field1."' ,bawara_field2='".$field2."',bawara_field3='".$field3."',bawara_field4='".$field4."' WHERE user_id = ".$user_id."";
    }else{
       echo $sql="INSERT INTO `shbawaras` (user_id,occupation,workplace,bawara_field1,bawara_field2,bawara_field3,bawara_field4) VALUES (".$user_id.",'".$occupation."','".$place."','".$field1."','".$field2."','".$field3."','".$field4."')";
    }
    //echo $sql;
    if($db->query($sql)){
...
if($work==61){
    ...
    $link= new Link();
    $fields=$_POST;
    if(!$current_user->authenticated) $return[0]= "failed";
    else{
        $link->author=$current_user->user_id;
        $link->status="published";
        $link->category=1;
        $link->title = sanitize(substr($fields['plan-title'], 0 ,40) , 3) ;
        $link->title_url = makeUrlFriendly($link->title);
        $link->link_summary = close_tags(stripslashes(sanitize($fields['plan-desc'], 4)));
        $link->link_field1=$fields['plan-interest'];
        $link->link_field2=date('Y-m-d H:i:s', $fields['plan-time']);
        $link->link_field3=$fields['plan-place'];
        $link->link_field12=urlencode("http://".$fields['plan-prom-link']);
        $link->link_field4=floatval($fields['plan-place-lat']);
        $link->link_field5=floatval($fields['plan-place-lng']);
        if(!isset($fields['plan-end-time'])) $fields['plan-end-time']= $fields['plan-time'];
        $link->link_field14=date('Y-m-d H:i:s', $fields['plan-end-time']);
        if(!isset($fields['plan-closing-time'])) $fields['plan-closing-time']= $fields['plan-time'];
        $link->link_field6=date('Y-m-d H:i:s', $fields['plan-closing-time']);
        $link->link_field11="public";
        $link->link_group_id=$fields['group_id'];
        $link->debug=false;
        if($link->store()){
            $link->canAccess=true;
            $return[0]="success";
            $return[1]=$link->print_summary('summary', true, 'link_summary.tpl');
        }else{
            $return[0]= "failed";
        }
    }
...
if($work==90){
    ...
    if(check_email($email) || isValidPhone($email)){
        $sql="INSERT INTO ".table_reg_key." (`reg_email`, `date`, `active`)  VALUES('".$email."', '".date('d-m-Y H:i:s', time())."', 0)";
        if($db->query($sql)){
            echo "success";
        }else echo "failed";
    }else echo "failed";
...
if($work==66){
	...
	$venue_id = sanitize(strip_tags($_POST['id1']));

	$name= sanitize(strip_tags($_POST['name']));
	$interest_name = sanitize(strip_tags($_POST['interest']));
	$desc = sanitize(strip_tags($_POST['desc']));
	$address = sanitize(strip_tags($_POST['address']));
	$street = sanitize(strip_tags($_POST['street']));
	$checkinscount = sanitize(strip_tags($_POST['checkinscount']));
	$phone = sanitize(strip_tags($_POST['phone1']));
	$interest_id = sanitize(strip_tags($_POST['interest_id']));
	$interest_id1 = sanitize(strip_tags($_POST['interest_id1']));
	$interest_id2 = sanitize(strip_tags($_POST['interest_id2']));
	$interest_id3 = sanitize(strip_tags($_POST['interest_id3']));
	$interest_id4= sanitize(strip_tags($_POST['interest_id4']));
	$interest_id5 = sanitize(strip_tags($_POST['interest_id5']));
	$interest_id6 = sanitize(strip_tags($_POST['interest_id6']));
	$interest_id7 = sanitize(strip_tags($_POST['interest_id7']));
	$interest_id8= sanitize(strip_tags($_POST['interest_id8']));
	$interest_id9 = sanitize(strip_tags($_POST['interest_id9']));
	$interest_id10 = sanitize(strip_tags($_POST['interest_id10']));
	$interest_id11 = sanitize(strip_tags($_POST['interest_id11']));
	$interest_id12 = sanitize(strip_tags($_POST['interest_id12']));
	$interest_id13 = sanitize(strip_tags($_POST['interest_id13']));
	$interest_id14 = sanitize(strip_tags($_POST['interest_id14']));
	...
	if(!is_null($user_id)){
		...
		global $db, $current_user;
		$sql="UPDATE `shvenues` SET name = '".$name."',checkinscount = ".$checkinscount.",description = '".$desc."',street = '".$street."',phone1 = ".$phone.",interest_name = '".$interest_name."' WHERE venue_id = ".$venue_id." ";
		echo $sql;
	$interests = array($interest_id,$interest_id1,$interest_id2,$interest_id3,$interest_id4,$interest_id5,$interest_id6,$interest_id7,$interest_id8,$interest_id9,$interest_id10,$interest_id11,$interest_id12,$interest_id13,$interest_id14);
		print_r($interests[1]);
					for($i=0;$i<15;$i++)
					{				
						if(!is_null($interests[$i])) {
								$sql1="INSERT INTO `shvenue_interest`(venue_id,interest_id) VALUES ('".$venue_id."',".$interests[$i].")";
								echo $sql1;
								//$db->query($sql1);
						}
					}	
...
if($work==67){
	...
	$venue_id = sanitize(strip_tags($_POST['venue_id']));
	if(!is_null($user_id)){

		global $db, $current_user;
		$sql="DELETE FROM `shvenues` WHERE venue_id = '".$venue_id."' ";
		echo $sql;
	}
