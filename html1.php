...
function keyEncoding($key,$value){
    return base64_encode($key.'-'.$value.'-shaukk');
}
function keyDecoding($key,$value){
    $string = base64_decode($value);
   // echo "....".strpos($string, '-s')."....";
    return trim(substr($string, (strlen($key)+1), (strlen($string)-strlen($key)-strlen('shaukk')-2)));
}

function send_emailSmtp($to,$name,  $subject, $body){
...
function getInterestImage($id, $size){
    global $db;
    //echo "..".$id."..";
    if(!is_numeric($id))die();
    $interestExist=$db->get_var('SELECT COUNT(*) FROM '.table_interests." WHERE interest_id=".$id);
    if($interestExist<=0) return false;
    if($size=="original")  return $src= 'images/interest/'.$id.'.jpg';
    return $src= 'images/interest/'.$id.'_'.$size.'.jpg';
    //return 'images.php?url='.urlencode($src);
}
...
function related_stories($storyid, $related_tags, $category){
	...
	$stories = array();
        foreach($related_story as $id => $rs){
	        $sql = "SELECT link_id, link_category, link_title, link_title_url FROM ".table_links." WHERE link_id={$rs['tag_link_id']}";
		if ($rs2 = $db->get_row($sql,ARRAY_A))
		{
			$related_story[$id]=array_merge($related_story[$id],$rs2);
			if ($rs2['link_title_url'] == ""){
				$related_story[$id]['url'] = getmyurl("story", $related_story[$id]['link_id']);
			} else {
				$related_story[$id]['url'] = getmyurl("storyURL", cat_safe_name($related_story[$id]['link_category']), urlencode($related_story[$id]['link_title_url']), $related_story[$id]['link_id']);
			}
...
function get_avatar($size = "large", $avatarsource, $user_name = "", $user_email = "", $user_id=""){
	...
	if(!$user->read()) {
	   ...
	   if($size == "large"){return my_base_url . my_pligg_base . "/avatars/Gravtar_M_30.png";}
       if($size == "small"){return my_base_url . my_pligg_base . "/avatars/Gravatar_M_15.png";}
       if($size == "big"){return my_base_url . my_pligg_base . "/avatars/Gravatar_M.png";}
       if($size == "50"){return my_base_url . my_pligg_base . "/avatars/Gravatar_M.png";}
       else{return my_base_url . my_pligg_base . "/avatars/Gravatar_M.png";}
	...
	if($size == "large"){$imgsize = Avatar_Large;}elseif($size=='big') {$imgsize = 85;}elseif($size=='50') {$imgsize = 50;}elseif($size=='250'){$imgsize = '250';}else{$imgsize = Avatar_Small;}
	...
	if(Enable_User_Upload_Avatar == true){
		...
		elseif (file_exists(mnmpath.'avatars/user_uploaded/'. $user_name . "_" . $imgsize . ".jpg"))
		{
            ...
            $imgsrc = my_base_url . my_pligg_base . '/avatars/user_uploaded/' . $user_id . "_original.jpg";
		    return latest_avatar($imgsrc, mnmpath.'avatars/user_uploaded/'.$user_name . "_original.jpg");
        }

    }
	if($size == "large"){return my_base_url . my_pligg_base . '/avatars/Gravatar_'.$sex.'_30.png';}
	if($size == "small"){return my_base_url . my_pligg_base . '/avatars/Gravatar_'.$sex.'_15.png';}
	if($size == "big"){return my_base_url . my_pligg_base . '/avatars/Gravatar_'.$sex.'.png';}
	if($size == "50"){return my_base_url . my_pligg_base . '/avatars/Gravatar_'.$sex.'.png';}
	else{return my_base_url . my_pligg_base . '/avatars/Gravatar_'.$sex.'.png';}
}
...
function getmyurl($x, $var1="", $var2="", $var3="") {
	...
	if ($URLMethod == 1) {
		...
		If ($x == "profile") {return my_pligg_base."/profile.php?login=".$var1;}
		If ($x == "profileId") {return my_pligg_base."/profile.php?id=".$var1;}
