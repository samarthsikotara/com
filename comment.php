...
class Comment {
	...
	function store() {
		...
		if($this->id===0) {
			...
			$comment_status = $this->status;
            if($this->author==0) die("You must be logged in to comment.");
			if($this->canSave == true){
                ...
                $users_array[1]['time']=date("Y-m-d H:i:s", (time()+19800));
                //echo $this->type;
                $type="plan";
                addNotification('comment', $type,$comment_link, $users_array);
                addSubscriber('comment', $type,$comment_link,$current_user->user_id);
	...
	function fill_smarty($smarty){
		...
		$smarty->assign('author_name', $this->comment_author_name);
        ...
        }else$smarty->assign('is_author', 0);

		$smarty->assign('comment_link', $this->link);
		$smarty->assign('user_view_url', getmyurl('profileId', $this->author));
	...
	function username() {
		...
		if ($user = $db->get_row("SELECT * FROM " . table_users . " WHERE user_id = $this->author")) {
            ...
            $this->comment_author_name=$user->user_names;
		}
