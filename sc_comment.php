...
class Sc_comment {
	...
	function store() {
		...
		$comment_scribble = $this->scribble;
		...
		if($this->id===0) {
			...
			if($this->canSave == true){
				...
				$sql = "INSERT IGNORE INTO " . table_scribble_comments . " (comment_parent, comment_user_id, comment_scribble_id, comment_karma, comment_date, comment_randkey, comment_content, comment_status) VALUES ($comment_parent, $comment_author, $comment_scribble, $comment_karma, FROM_UNIXTIME($comment_date), $comment_randkey, '$comment_content', '$comment_status')";
				//echo $sql;
                $db->query($sql);
			...
			$sql = "UPDATE " . table_scribble_comments . " set comment_votes=$comment_votes, comment_user_id=$comment_author, comment_scribble_id=$comment_scribble, comment_karma=$comment_karma, comment_date=FROM_UNIXTIME($comment_date), comment_randkey=$comment_randkey, comment_content='$comment_content', comment_status='$comment_status' WHERE comment_id=$comment_id";
			$db->query($sql);
		}
        return true;
	}
	function read() {
		...
		if(!is_numeric($id)){return false;}
			...
			$scribble = $db->get_row("SELECT * FROM " . table_scibble_comments . " WHERE comment_id = $id");

        if($scribble) {
			$this->author=$scribble->comment_user_id;
			$this->randkey=$scribble->comment_randkey;
			$this->scribble=$scribble->comment_scribble_id;
			$this->karma=$scribble->comment_karma;
			$this->parent=$scribble->comment_parent;
			$this->content=$scribble->comment_content;
			$this->status=$scribble->comment_status;
			$this->randkey=$scribble->comment_randkey;
			$this->votes=$scribble->comment_votes;
			$date=$scribble->comment_date;
	...
	function quickread() {
		...
		$id = $this->id;
		if(!is_numeric($id)){return false;}
		if(($scribble = $db->get_row("SELECT * FROM " . table_scribble_comments . " WHERE comment_id = $id"))) {
			$this->content=$scribble->comment_content;
			return $scribble->comment_content;
		}
	...
	function fill_smarty($smarty){
		global $current_user, $the_template, $comment_counter, $link, $ranklist;
		...
		$smarty->assign('author_name', $vars['comment_username']);
	...
	function username() {
		global $db;
		if ($user = $db->get_row("SELECT * FROM " . table_users . " WHERE user_id = $this->author")) {
			$this->username = $user->user_login;
			$this->userkarma = $user->user_karma;
