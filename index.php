...
$bawraas_id = $_GET['user_id'];

$sql="select user_url FROM `shusers` WHERE user_id= ".$bawraas_id."";
$result=$db->get_var($sql);

$url=getmyFullUrl('bawraas_profiles',$result);

$main_smarty->assign('id', $bawraas_id);
...
// show the template
    if(isset($_REQUEST['data']) && $_REQUEST['data']=='scribble'){
        ...
        $main_smarty->assign('tpl_center', $the_template . '/index_new_center');
        $main_smarty->display($the_template . '/pligg.tpl');
    }
?>
<!--
